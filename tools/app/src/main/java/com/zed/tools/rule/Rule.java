...
public class Rule extends View {
    ...
    public Rule(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//        setBackground(createBackground());
    ...
    private void drawLine(Canvas canvas) {
        ...
        canvas.drawLine(getPaddingStart(), mHeight - dateHeight * 2 - getPaddingBottom(), mWidth - getPaddingEnd(), mHeight - dateHeight * 2 - getPaddingBottom(), paint);
    }
        ...
        for (int i = 0; drawCount <= 4 * width; i++) {
            ...
            if (xPosition + getPaddingRight() < mWidth) {
                if ((mValue + i) % mModType == 0) {
                    //右边的大刻度
                    canvas.drawLine(xPosition, mHeight - mDensity * ITEM_MAX_HEIGHT - dateHeight * 2 - getPaddingBottom(), xPosition, mHeight - dateHeight * 2 - getPaddingBottom(), linePaint);
                    if (mValue + i <= mMaxValue) {
                        switch (mModType) {
                            case MOD_TYPE_HALF:
                                ...
                                canvas.drawText(drawStrY, xPosition - boundsY.width() / 2, mHeight - boundsY.height() - getPaddingBottom(), paintText);
                                canvas.drawText(drawStrD, xPosition - boundsD.width() / 2, mHeight - getPaddingBottom(), paintText);
                                break;
                            case MOD_TYPE_ONE:
                    ...
                    //右边的小刻度
                    canvas.drawLine(xPosition, mHeight - mDensity * ITEM_MIN_HEIGHT - dateHeight * 2 - getPaddingTop(), xPosition, mHeight - dateHeight * 2 - getPaddingTop(), linePaint);
                }
            }
            xPosition = (width / 2 - mMove) - i * mLineDivider * mDensity;
            if (xPosition > getPaddingLeft()) {
                if ((mValue - i) % mModType == 0) {
                    //左边的大刻度
                    canvas.drawLine(xPosition, mHeight - dateHeight * 2 - mDensity * ITEM_MAX_HEIGHT - getPaddingBottom(), xPosition, mHeight - dateHeight * 2 - getPaddingBottom(), linePaint);
                    if (mValue - i >= 0) {
                        switch (mModType) {
                            case MOD_TYPE_HALF:
                                String drawStrY = "2018-07-09";
                                Rect boundsY = getTextRect("2018-07-09");
                                String drawStrD = "22:36:00";
                                Rect boundsD = getTextRect(drawStrD);
                                canvas.drawText(drawStrY, xPosition - boundsY.width() / 2, mHeight - boundsD.height() - getPaddingBottom(), paintText);
                                canvas.drawText(drawStrD, xPosition - boundsD.width() / 2, mHeight - getPaddingBottom(), paintText);
                                break;
                            case MOD_TYPE_ONE:
                                canvas.drawText(String.valueOf(mValue - i), xPosition - (textWidth * numSize / 2), getHeight() - textWidth, textPaint);
                    ...
                    //左边的小刻度
                    canvas.drawLine(xPosition, mHeight - dateHeight * 2 - mDensity * ITEM_MIN_HEIGHT - getPaddingTop(), xPosition, mHeight - dateHeight * 2 - getPaddingTop(), linePaint);
                }
            }
    ...
    private void drawMiddleLine(Canvas canvas) {
        ...
        canvas.drawLine(mWidth / 2, 0, mWidth / 2, mHeight - dateHeight * 2 - getPaddingBottom(), redPaint);
        ...
        canvas.drawLine(mWidth / 2, mHeight - indexTitleHight - dateHeight * 2 - getPaddingBottom(), mWidth / 2, mHeight - dateHeight * 2 - getPaddingBottom(), ovalPaint);
        Paint shadowPaint = new Paint();
        shadowPaint.setStrokeWidth(shadow);
        shadowPaint.setColor(Color.parseColor(color));
        canvas.drawLine(mWidth / 2 + gap, 0, mWidth / 2 + gap, mHeight - dateHeight * 2 - getPaddingBottom(), shadowPaint);
        canvas.restore();
    }
